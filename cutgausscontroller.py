...
class CutGaussController(BaseGaussController):
    ...
    def updateParams(self):
        ...
        try:
            ...
            wx.MessageBox("Ошибка формата в поле 't0'", "Ошибка", wx.OK | wx.ICON_ERROR)
            return False
    ...
    @property
    def title(self):
        return "Гауссов импульс со ступенькой"
    ...
    def function(self, time):
        ...
        return numpy.array(
            [
                1.0
                / (self._sigma * numpy.sqrt(2.0 * numpy.pi))
                * numpy.exp(-((t - self._mu) ** 2) / (2.0 * (self._sigma**2)))
                if t < self._t0
                else 0
                for t in time
            ]
